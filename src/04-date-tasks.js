...
function angleBetweenClockHands(date) {
  ...
  const diffDegrees = Math.abs(hourAngle - minuteAngle);
  const minAngle = Math.min(diffDegrees, 360 - diffDegrees);
  const result = (minAngle * Math.PI) / 180;
  return result;
}
